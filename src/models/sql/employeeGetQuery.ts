...
const employeeGetQuery = async () => {
  ...
  const loginInfoQuery = `SELECT a.id , b.pass, d.departmentColor FROM ${tables.employeeTable} a INNER JOIN ${tables.passTable} b ON a.passId = b.id INNER JOIN belongs c ON a.id = c.employeeId INNER JOIN departments d ON c.departmentId = d.id`
